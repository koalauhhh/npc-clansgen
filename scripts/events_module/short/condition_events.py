...
class Condition_Events:
    ...
    def handle_already_ill(cat):
        ...
        for illness in illnesses:
            ...
            if cat.dead or (
                cat.status.is_leader and starting_life_count != cat_clan.leader_lives
            ):
                try:
            ...
            if cat.status.is_leader and starting_life_count != cat_clan.leader_lives:
                break

            # heal the cat, other clan cats heal immediately
            elif cat.healed_condition is True or not cat.status.alive_in_player_clan:

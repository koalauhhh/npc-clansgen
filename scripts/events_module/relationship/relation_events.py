...
class Relation_Events:
    ...
    def romantic_events(cat):
        ...
        if use_mate or cat.no_mates:
            cat_to_choose_from = [
                ...
                for mate_id in cat.mate
                if cat.all_cats[mate_id].status.is_outsider
            ]
    ...
    def group_events(cat):
        ...
        possible_interaction_cats = list(
            filter(
                lambda cat: (cat.status.is_outsider),
                Cat.all_cats.values(),
            )
        )
    ...
    def cats_with_relationship_constraints(main_cat, constraint):
        ...
        cat_list = list(
            filter(
                lambda cat: (not cat.status.is_outsider and not (cat.status.group != main_cat.status.group)),
                Cat.all_cats.values(),
            )
        )

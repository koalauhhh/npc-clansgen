...
class Pregnancy_Events:
    ...
    def handle_adoption(cat: Cat, other_cat=None, clan=game.clan):
        ...
        if other_cat and (
            other_cat.status.is_outsider or other_cat.birth_cooldown > 0
        ):
            return
        ...
        if cat.status.group.is_other_clan_group():
            game.cur_events_list.append(
                Single_Event(print_event, ["birth_death", "other_clans"], cat_dict=cats_involved)
            )
        else:
            game.cur_events_list.append(
        ...
        if get_clan_setting("same sex birth"):
            ...
            if not other_cat and random.randint(0, 1):
                ...
                if cat.status.group.is_other_clan_group():
                    game.cur_events_list.append(
                        Single_Event(
                            print_event, ["birth_death", "other_clans"], cats_involved, cat_dict=cat_dict
                        )
                    )
                else:
                    game.cur_events_list.append(
            ...
            if cat.status.group.is_other_clan_group():
                game.cur_events_list.append(
                    Single_Event(text, ["birth_death", "other_clans"], cat.ID, cat_dict={"m_c": cat})
                )
            else:
                game.cur_events_list.append(
            ...
            if not other_cat and cat.gender == "male":
                ...
                if cat.status.group.is_other_clan_group():
                    game.cur_events_list.append(
                        Single_Event(
                            print_event, ["birth_death", "other_clans"], cats_involved, cat_dict={"m_c": cat}
                        )
                    )
                else:
                    game.cur_events_list.append(
            ...
            if cat.status.group.is_other_clan_group():
                game.cur_events_list.append(
                    Single_Event(
                        text, ["birth_death", "other_clans"], pregnant_cat.ID, cat_dict={"m_c": cat}
                    )
                )
            else:
                game.cur_events_list.append(
    ...
    def handle_two_moon_pregnant(cat: Cat, clan=game.clan):
        ...
        if not int(
            ...
            if cat_clan.leader_lives > 1 and cat.status == "leader":
        ...
        elif not cat.status.is_outsider and not cat.status.group.is_other_clan_group() or not (cat.status.get_last_living_group()).is_other_clan_group():  # if cat doesn't die, give recovering from birth
        ...
        if cat.status.group.is_other_clan_group() or (cat.status.get_last_living_group()).is_other_clan_group():
            game.cur_events_list.append(
                Single_Event(
                    print_event, ["health", "birth_death", "other_clans"], involved_cats, cat_dict=cat_dict
                )
            )
        else:
            game.cur_events_list.append(
    ...
    def get_kits(
        ...
        if cat:
            cat_clan = get_cat_clan(cat.status.group)
        elif adoptive_parents:
            cat_clan = get_cat_clan((Cat.fetch_cat(adoptive_parents[0])).status.group)
        all_kitten = []
        ...
        for kit in range(kits_amount):
            if not cat:
                ...
                kit = Cat(parent1=blood_parent.ID, moons=0, backstory=backstory, status_dict={"social":Cat.fetch_cat(adoptive_parents[0]).status.social, "group":Cat.fetch_cat(adoptive_parents[0]).status.group, "rank":CatRank.NEWBORN})

            elif cat and other_cat:
                # Two parents provided
                # The cat that gave birth is always parent1 so there is no need to check gender
                kit = Cat(parent1=cat.ID, parent2=other_cat.ID, moons=0, status_dict={"social":cat.status.social, "group":cat.status.group, "rank":CatRank.NEWBORN})
                kit.thought = i18n.t("hardcoded.new_kit_thought", name=str(cat.name))
                kit.thought = event_text_adjust(Cat, kit.thought, random_cat=cat)
            else:
                # A one blood parent litter is the only option left.
                kit = Cat(parent1=cat.ID, moons=0, backstory=backstory, status_dict={"social":cat.status.social, "group":cat.status.group, "rank":CatRank.NEWBORN})
                kit.thought = i18n.t("hardcoded.new_kit_thought", name=str(cat.name))
                kit.thought = event_text_adjust(Cat, kit.thought, random_cat=cat)
            ...
            for cat_id in clan.clan_cats:
                ...
                if the_cat.dead or the_cat.status.group != kit.status.group:
                    continue
            ...
            kit.pelt.accessory = []
            clan.add_cat(kit)
            if kit.status.group.is_other_clan_group():
                cat_clan.add_to_clan(kit)
        ...
        for kitten in all_kitten:
            ...
            for second_kitten in all_kitten:
                ...
                if second_kitten.ID == kitten.ID:
                    continue
                start_relation = Relationship(kitten, second_kitten)
                start_relation.platonic_like += 20 + y
    ...
    def get_balanced_kit_chance(
        ...
        living_cats = len(
            [i for i in Cat.all_cats.values() if not i.status.is_outsider]
        )
        if living_cats < 10:
        ...
        settings_allow = (
            not second_parent and not get_clan_setting("single parentage")
        )

...
class Events:
    ...
    def one_moon(self):
        ...
        for cat in Cat.all_cats.copy().values():
            ...
            if cat.status.is_outsider and not cat.dead:
                self.one_moon_outside_cat(cat)
            elif cat.status.is_clancat or cat.status.group.is_afterlife():
        ...
        if Cat.dead_cats:
            ...
            for ghost in Cat.dead_cats:
                if ghost.status.group != CatGroup.PLAYER_CLAN:
                    pass
            ...
            if x.status.rank in [
                ...
                if x.moons >= 15:
                    if x.status.rank == CatRank.MEDICINE_APPRENTICE:
                        self.ceremony(x, CatRank.MEDICINE_CAT, game.clan)
                    elif x.status.rank == CatRank.MEDIATOR_APPRENTICE:
                        self.ceremony(x, CatRank.MEDIATOR, game.clan)
                    else:
                        self.ceremony(x, CatRank.WARRIOR, game.clan)
                elif not x.status.rank.is_any_apprentice_rank() and x.moons >= 6:
                    self.ceremony(x, CatRank.APPRENTICE, game.clan)
    ...
    def perform_ceremonies(self, cat, cat_clan):
        ...
        if cat_clan.leader:
            ...
            leader_outside = cat_clan.leader.status.is_outsider
        else:
            leader_dead = True
        ...
        if cat_clan.deputy:
            if (
                ...
                and cat_clan.deputy.status.alive_in_player_clan
                ...
                if cat.status.group.is_other_clan_group() or (cat.status.get_last_living_group()).is_other_clan_group():
                    game.cur_events_list.append(
                        Single_Event(text, ["ceremony", "other_clans"], cat_clan.deputy.ID)
                    )
                else:
                    game.cur_events_list.append(
        ...
        if not cat_dead:
            if cat.status.rank == CatRank.DEPUTY and cat_clan.deputy is None:
                cat_clan.deputy = cat
            if (
                cat.status.rank == CatRank.MEDICINE_CAT
                and cat_clan.medicine_cat is None
            ):
            ...
            if (
                ...
                if cat.moons > 140 or not int(
                    ...
                    if cat.status.rank == CatRank.DEPUTY:
                        cat_clan.deputy = None
                    self.ceremony(cat, CatRank.ELDER, cat_clan)
            ...
            if cat.moons == cat_class.age_moons[CatAge.ADOLESCENT][0]:
                if cat.status.rank == CatRank.KITTEN:
                    ...
                    if not has_med_app and not int(random.random() * chance):
                        self.ceremony(cat, CatRank.MEDICINE_APPRENTICE, cat_clan)
                        self.ceremony_accessory = True
                        self.gain_accessories(cat, cat_clan)
                    else:
                        ...
                        mediator_list = list(
                            filter(
                                ...
                                and x.status.group == cat.status.group,
                                Cat.all_cats_list,
                            )
                        )
                        ...
                        if (
                            ...
                            self.ceremony(cat, CatRank.MEDIATOR_APPRENTICE, cat_clan)
                            self.ceremony_accessory = True
                            self.gain_accessories(cat, cat_clan)
                        else:
                            self.ceremony(cat, CatRank.APPRENTICE, cat_clan)
                            self.ceremony_accessory = True
                            self.gain_accessories(cat, cat_clan)
            ...
            if cat.status.rank.is_any_apprentice_rank():
                ...
                if _ready:
                    ...
                    if cat.status.rank == CatRank.APPRENTICE:
                        self.ceremony(cat, CatRank.WARRIOR, game.clan, preparedness)
                        self.ceremony_accessory = True
                        self.gain_accessories(cat, cat_clan)

                    # promote to med cat
                    elif cat.status.rank == CatRank.MEDICINE_APPRENTICE:
                        self.ceremony(cat, CatRank.MEDICINE_CAT, game.clan, preparedness)
                        self.ceremony_accessory = True
                        self.gain_accessories(cat, cat_clan)

                    elif cat.status.rank == CatRank.MEDIATOR_APPRENTICE:
                        self.ceremony(cat, CatRank.MEDIATOR, game.clan, preparedness)
                        self.ceremony_accessory = True
                        self.gain_accessories(cat, cat_clan)
    ...
    def ceremony(self, cat, promoted_to, cat_clan, preparedness="prepared"):
        ...
        try:
            ...
            for c in cat.former_mentor:
                if Cat.fetch_cat(c).status.group == cat.status.group:
            ...
            for p in [cat.parent1, cat.parent2]:
                if Cat.fetch_cat(p):
                    ...
                    # who are also the leader are not counted.
                        ...
                        Cat.fetch_cat(p).status.group == cat.status.group
        ...
        alive_cats = list(
            filter(
                lambda kitty: (
                    ...
                    and kitty.status.group == cat.status.group
                ),
                Cat.all_cats.values(),
            )
        )
    ...
    def check_and_promote_leader(self):
        ...
        for iter_clan in (game.clan.all_clans + [game.clan]):
            # check for leader
            if iter_clan.leader:
                leader_invalid = not iter_clan.leader.status.is_clancat
            else:
                leader_invalid = True
            ...
            if leader_invalid:
                ...
                if iter_clan.leader:
                    ...
                    leader_outside = iter_clan.leader.status.is_outsider
                else:
                    leader_dead = True
                    leader_outside = True
    ...
    def check_and_promote_deputy(self):
        ...
        for iter_clan in (game.clan.all_clans + [game.clan]):
            if (
                ...
                or not iter_clan.deputy.status.is_clancat
                or iter_clan.deputy.status.rank == CatRank.ELDER
            ):
                if (iter_clan is game.clan) and not get_clan_setting("deputy"):
                ...
                possible_deputies = list(
                    filter(
                        lambda x: x.status.is_clancat
                        and get_cat_clan(x.status.group) == iter_clan
                ...
                if possible_deputies:
                    ...
                    if iter_clan.leader:
                        if not iter_clan.leader.status.is_clancat:
                    ...
                    if iter_clan.deputy:
                        if not iter_clan.deputy.status.is_clancat:
                ...
                if iter_clan != game.clan:
                        game.cur_events_list.append(Single_Event(text, ["ceremony", "other_clans"], involved_cats))
                else:

...
def get_alive_outside_queens(living_cats):
    living_kits = [
        cat
        for cat in living_cats
        if cat.status.is_outsider and cat.status.rank.is_baby()
    ]
    ...
    for cat in living_kits.copy():
        ...
        parents = [
            cat.fetch_cat(i)
            for i in parents
            if cat.fetch_cat(i) and cat.fetch_cat(i).status.is_outsider
        ]
        if not parents:
            continue
...
def get_alive_oc_queens(living_cats):
    living_kits = [
        cat
        for cat in living_cats
        # TODO: create status.alive_in_other_clan
        if cat.status.alive_in_player_clan and cat.status.rank.is_baby()
    ]
...
def get_cats_same_age(Cat, cat, age_range=10):
    ...
    for inter_cat in Cat.all_cats.values():
        if not inter_cat.status.is_clancat:
            continue
...
def create_new_cat_block(
    ...
    # get cat from other clans
    if "former Clancat" in attribute_list and rank in (CatRank.KITTEN, CatRank.NEWBORN, CatRank.MEDICINE_CAT):
        existing_clancats = [
            i for i in Cat.all_cats.values() if (get_cat_clan(i.status.group).name == event.other_clan.name)
        ]
        possible_clancats = []
        for cat in existing_clancats:
            if rank and rank != cat.status.rank:
                continue
            if gender and gender != cat.gender:
                continue
            possible_clancats.append(cat)
...
def get_cat_clan(group):
    """
    returns the clan object of given group
    """
    clans_list = list(zip(game.clan.other_clans, game.clan.all_clans))
    if group == CatGroup.PLAYER_CLAN:
        return game.clan
